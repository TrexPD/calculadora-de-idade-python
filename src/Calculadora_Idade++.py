...
def dia_semana(dia = datetime.today().strftime("%d"),
...
def data_correta() -> list():
    ...
    while True:
        ...
        if len(str(ano)) == 4 and ano <= ano_atual:
            ...
            print("\033[1;31mFormato inválido! O ano tem que ter 4 digitos e igual ou menor que o ano atual! [Ex: 1900]\033[m")
    return [dia, mes, ano]
...
q_anos: int = abs(q_dias - (num_anos_bissextos - 1)) / 365      # Acho que eles não consideram o dia em que a pessoa nasceu um ano bissexto(mesmo sendo!)
...
if data_atual_string >= niver_atual:
    ...
    if meses_atual != mes:
        for i in range(contador_meses, (meses_atual + 1)):
    ...
    niver_anterior = datetime.strptime(f"{ano_anterior}-{mes}-{dia}", "%Y-%m-%d")
...
if len(lista_meses) > 1:
    ...
    for i in lista_meses:
        if i >= 28 and lista_meses[-1] != ultimo_mes:
            q_meses_restantes += 1 
        else:
            q_dias_restantes += i
